...
def customer_add_order(request):
    ...
    if request.method == "POST":
        ...
        # Check wether customer has any order that is not delivered
...
def driver_pick_order(request):
    ...
    if request.method == "POST":
        ...
        #if Order.objects.filter(driver = driver).exclude(status = Order.ONTHEWAY and Order.DELIVERED):
        if Order.objects.filter(driver = driver).filter(status = Order.ONTHEWAY):
        ...
        try:
            ...
            if order:
                return JsonResponse({"status": "failed", "error": "This order is still COOKING."})
